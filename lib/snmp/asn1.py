...
class Encoder:
    ...
    @contextmanager
    def enter(self, nr: TNumber, cls: Optional[TClass] = None) \
            -> Iterator[None]:
    ...
    def write(
        ...
        """This method encodes one ASN.1 tag and writes it to the output
        buffer.

        Note:
            Normally, ``value`` will be the only parameter to this method.
            In this case Python-ASN1 will autodetect the correct ASN.1 type
            from the type of ``value``, and will output the encoded value based
            on this type.
        ...
        Args:
            ...
            typ (int): This optional parameter can be used to write constructed
                ...
                encoding type. In this case, ``value`` must already be valid
                ASN.1 encoded data as plain Python bytes. This is not normally
                how constructed types should be encoded though,
                see `Encoder.enter()` and `Encoder.leave()` for the recommended
                way of doing this. Use ``Type`` enumeration.
        ...
        if nr is None:
            if isinstance(value, int):
                ...
                raise Error(
                    f"Cannot determine Number for value type {type(value)}")
        if typ is None:
            typ = Type.Primitive
        if cls is None:
            cls = Class.Universal
    ...
    def _encode_object_identifier(self, oid: tuple) -> bytes:
