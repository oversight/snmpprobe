...
def host_resources_mib(state_data):
    if 'processor' in state_data:
        cpus = [item.get('hrProcessorLoad', 0)
                for item in state_data['processor'].values()]
        aggr = sum(cpus) / len(cpus) if cpus else 0
        state_data['cpu'] = {
            'cpuAverage': {'name': 'cpuAverage', 'percentUsed': aggr}}
