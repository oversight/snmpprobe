...
from lib.check.base import Base
...
class TestProbe(unittest.TestCase):
    def test_check(self):
        ...
        asyncio.run(Base.run(data, TEST_CREDENTIALS))
    ...
    def test_snmp_func(self):
        ...
        asyncio.run(Base.on_run_check(data))

    def test_my_check(self):
        check_name = 'MyCheck'
        check_conf = {'types': []}
        check = make_check(mi, check_name, check_conf)
        NEW_CHECK_DEFINITIONS[check_name] = check
        ...
        asyncio.run(Probe.on_run_check(data))
